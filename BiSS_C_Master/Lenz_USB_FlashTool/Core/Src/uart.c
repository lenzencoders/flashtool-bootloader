...
static uint32_t elapsed_global = 0;
static uint32_t elapsed_led_global = 0;
		...
		switch (command) {
			...
			case UART_COMMAND_CHECK_PROGRAM_CRC32:
				...
				if(Program_CRC32 != UartBank1.ProgramCRC32){
					...
					UartSetMemoryState(UART_MEMORYSTATE_FW_CHECK_CRC32_OK);
				}
				queue_read_cnt = (queue_read_cnt + 1U) % QUEUE_SIZE;
				queue_cnt--;
				retry_cnt = 0;
			...
			case UART_COMMAND_WRITE_CURRENT_PAGE_CRC32:
				if(cmd_data_len == 4) {
			...
			case UART_COMMAND_READ_PROGRAM_VER:
				UART_TX.cmd = UART_COMMAND_READ_PROGRAM_VER;
				UART_TX.len = cmd_data_len;
				UART_TX.adr_h = 0; 
				UART_TX.adr_l = 0;
				uint32_t *value_ptr = (uint32_t*)(PROGRAM_VER_ADR);
				uint32_t value = *value_ptr;
				UART_TX.Buf[3] = (value >> 0) & 0xFF;
				UART_TX.Buf[2] = (value >> 8) & 0xFF;
				UART_TX.Buf[1] = (value >> 16) & 0xFF;
				UART_TX.Buf[0] = (value >> 24) & 0xFF;
				UART_Transmit(&UART_TX);
				queue_read_cnt = (queue_read_cnt + 1U) % QUEUE_SIZE;
				queue_cnt--;
				retry_cnt = 0;
...
void UART_StateMachine(void)
{
  ...
  LL_LPUART_EnableDMAReq_TX(UART_BL);
	...
	if(GetTampFlag(TAMP_FLAGS_STAY_MAIN_FW)){
		DeInit();
		StartProgram(PROGRAM_ADR);
	}
